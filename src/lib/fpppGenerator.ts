...
export function terbilang(n: number): string {
  ...
  const hasil = kekata(Math.floor(n));
...
export async function generateFPPP(submission: any | null, isEmpty: boolean = false) {
  ...
  const drawSidebarText = (text: string, startY: number, height: number) => {
    ...
    // Draw rotated text (angle 90 or 270)
    // 270 degrees is oriented upwards, which matches standard layout
  ...
  const submitDateStr = (!isEmpty && submission?.createdAt) 
    ? format(parseFirestoreDate(submission.createdAt), 'dd MMMM yyyy', { locale: { code: 'id' } as any })
    : '';
  ...
  const isRutin = !isEmpty && (submission?.title?.toLowerCase().includes('rutin') || submission?.description?.toLowerCase().includes('rutin'));
  const isAset = !isEmpty && (submission?.title?.toLowerCase().includes('aset') || submission?.description?.toLowerCase().includes('aset') || submission?.title?.toLowerCase().includes('beli') || submission?.description?.toLowerCase().includes('beli'));
  ...
  const isTunai = !isEmpty && (submission?.sumberRekening?.toLowerCase().includes('tunai') || submission?.title?.toLowerCase().includes('tunai') || submission?.description?.toLowerCase().includes('tunai'));
  ...
  if (!isEmpty && submission) {
    ...
    const formattedAmount = `Rp ${submission.amount.toLocaleString('id-ID')},-`;
    doc.text(formattedAmount, 67, 84.5);
    
    const textTerbilang = terbilang(submission.amount);
    doc.setFontSize(8);
  ...
  const matchDesc = (regex: RegExp) => !isEmpty && submission && (regex.test(submission.title.toLowerCase()) || regex.test(submission.description.toLowerCase()));
  
  const isInvoice = matchDesc(/invoice|faktur|tagihan/);
  const isFakturPajak = matchDesc(/pajak|tax|pph|ppn/);
  const isKwitansi = matchDesc(/kwitansi|kuitansi|bukti bayar|nota/)|| true; // default checklist is typical
  ...
  doc.setFontSize(8);
  // Drawing clean underline for signature names
  ...
  if (!isEmpty && submission?.signatures) {
    ...
    if (submission.signatures.roni) {
      try {
        ...
        doc.setFontSize(7.5);
        doc.setFont("helvetica", "bold");
        doc.text("Ttd digital: M. Roni", 120, 196);
        doc.setFont("helvetica", "normal");
        const roniDate = format(parseFirestoreDate(submission.signatures.roni.timestamp), 'dd/MM/yyyy HH:mm');
        doc.text(`Waktu: ${roniDate}`, 120, 200);
      } catch (err) {
      ...
      try {
        ...
        doc.setFontSize(7.5);
        doc.text("DISETUJUI SECARA DIGITAL oleh M. Roni", 120, 214);
      } catch (err) {
        doc.text("Approved (M. Roni)", 75, 214);
      }
    }

    // If kamal signed (Kepala SCB)
    if (submission.signatures.kamal) {
      try {
        doc.addImage(submission.signatures.kamal.signature, 'PNG', 75, 222, 32, 11);
        doc.setFontSize(7.5);
        doc.setFont("helvetica", "bold");
        doc.text("Disetujui: Ahmad Kamal (Kepala)", 120, 227);
        doc.setFont("helvetica", "normal");
        const kamalDate = format(parseFirestoreDate(submission.signatures.kamal.timestamp), 'dd/MM/yyyy HH:mm');
        doc.text(`Waktu: ${kamalDate}`, 120, 231);
      } catch (err) {
  ...
  const payDateStr = (!isEmpty && submission?.bookedAt) 
    ? format(parseFirestoreDate(submission.bookedAt), 'dd MMMM yyyy', { locale: { code: 'id' } as any })
    : "";
  ...
  if (!isEmpty && submission?.isBooked) {
    ...
    doc.setFont("helvetica", "normal");
    const bookDate = submission.bookedAt ? format(parseFirestoreDate(submission.bookedAt), 'dd/MM/yy') : "";
  ...
  const filename = isEmpty 
    ...
    : `FPPP_Pengajuan_${submission?.title?.replace(/\s+/g, '_') || submission?.id}.pdf`;
